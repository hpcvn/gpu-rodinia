...
void initialize_variables(int nelr, float* variables)
{
	...
	hipError_t error;
	hipLaunchKernelGGL(cuda_initialize_variables, Dg, Db, 0, 0, nelr, variables);
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors)
{
  	hipError_t error;
	dim3 Dg(nelr / block_length), Db(block_length);
	hipLaunchKernelGGL(cuda_compute_step_factor, Dg, Db, 0, 0, nelr, variables, areas, step_factors);			    error = hipGetLastError();
		    if (error != hipSuccess) 
		      {
...
void compute_flux_contributions(int nelr, float* variables, float* fc_momentum_x, float* fc_momentum_y, float* fc_momentum_z, float* fc_density_energy)
{
	dim3 Dg(nelr / block_length), Db(block_length);
	hipError_t error;
	hipLaunchKernelGGL(cuda_compute_flux_contributions, Dg, Db, 0, 0, nelr, variables, fc_momentum_x, fc_momentum_y, fc_momentum_z, fc_density_energy);
		    error = hipGetLastError();
		    if (error != hipSuccess) 
		      {
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fc_momentum_x, float* fc_momentum_y, float* fc_momentum_z, float* fc_density_energy, float* fluxes)
{
	hipError_t error;
	dim3 Dg(nelr / block_length), Db(block_length);
	hipLaunchKernelGGL(cuda_compute_flux, Dg, Db, 0, 0, nelr, elements_surrounding_elements, normals, variables, fc_momentum_x, fc_momentum_y, fc_momentum_z, fc_density_energy, fluxes);
		    error = hipGetLastError();
		    if (error != hipSuccess) 
		      {
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes)
{
	hipError_t error;
	dim3 Dg(nelr / block_length), Db(block_length);
	hipLaunchKernelGGL(cuda_time_step, Dg, Db, 0, 0, j, nelr, old_variables, variables, step_factors, fluxes);
		    error = hipGetLastError();
		    if (error != hipSuccess) 
		      {
