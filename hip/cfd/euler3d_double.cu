...
{
	dim3 Dg(nelr / block_length), Db(block_length);
	hipLaunchKernelGGL(cuda_compute_step_factor, Dg, Db, 0, 0, nelr, variables, areas, step_factors);
	hipError_t error = hipGetLastError();		
	if (error != hipSuccess) 
	  {
...
{
	dim3 Dg(nelr / block_length), Db(block_length);
	hipLaunchKernelGGL(cuda_compute_flux, Dg, Db, 0, 0, nelr, elements_surrounding_elements, normals, variables, fluxes);
	hipError_t error = hipGetLastError();
	if (error != hipSuccess) 
	  {
