                           ...
                           int level)
{
    ...
    if (numBlocks > 1)
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<true, false>), grid, threads, sharedMemSize , 0, outArray, 
                                                                 inArray, 
        ...
        if (np2LastBlock)
        {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<true, true>), 1, numThreadsLastBlock, sharedMemLastBlock , 0,
        ...
        hipLaunchKernelGGL(uniformAdd, grid, threads , 0, 0, outArray, 
                                        g_scanBlockSums[level], 
        ...
        if (np2LastBlock)
        {
            hipLaunchKernelGGL(uniformAdd, 1, numThreadsLastBlock , 0, 0, outArray, 
                                                     g_scanBlockSums[level], 
                                                     numEltsLastBlock, 
                                                     numBlocks - 1, 
                                                     numElements - numEltsLastBlock);
    ...
    {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<false, false>), grid, threads, sharedMemSize , 0, outArray, inArray,
                                                                  0, numThreads * 2, 0, 0);
        CUT_CHECK_ERROR("prescan");
    }
    else
    {
         hipLaunchKernelGGL(HIP_KERNEL_NAME(prescan<false, true>), grid, threads, sharedMemSize , 0, outArray, inArray, 
                                                                  0, numElements, 0, 0);
