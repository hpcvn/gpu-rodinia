...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
    ...
    hipEventRecord( start, 0 );
        for (int i=0; i<NT; i++) {
            hipLaunchKernelGGL(vlc_encode_kernel_sm64huff, grid_size, block_size, sm_size, 0, d_sourceData, d_codewords, d_codewordlens,  
#ifdef TESTING
    ...
    hipLaunchKernelGGL(pack2, num_scan_elements/16, 16, 0, 0, (unsigned int*)d_destData, d_cindex, d_cindex2, (unsigned int*)d_destDataPacked, num_elements/num_scan_elements);
