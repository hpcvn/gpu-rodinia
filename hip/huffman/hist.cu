...
int runHisto(char* file, unsigned int* freq, unsigned int memSize, unsigned int *source) {
    ...
    for(int i = 0; i < totalNum; i+=partialNum*2)
    {
        ...
        hipLaunchKernelGGL(histo_kernel, blocks*2, 256, 0, stream0,  dev_buffer0, partSize, dev_histo );
        hipLaunchKernelGGL(histo_kernel, blocks*2, 256, 0, stream1,  dev_buffer1, partSize, dev_histo );
    }
