					   ...
					   unsigned int* nextqrychar)
{
    ...
    *alignments = (struct Alignment *) calloc(alignmentOffset, sizeof(Alignment));
	//hipMallocHost((void**)alignments, numAlignments * sizeof(Alignment));
}
                    ...
                    unsigned int numAlignments)
{
    ...
    hipLaunchKernelGGL(printKernel, dimGrid, dimBlock, 0 , 0, d_matches,
            numMatches,
            d_alignments,
...
void matchOnGPU(MatchContext* ctx, bool doRC)
{
    ...
    if (doRC) {
        ...
        hipLaunchKernelGGL(mummergpuRCKernel, dimGrid, dimBlock, 0 , 0, ctx->results.d_match_coords,
                ctx->queries->d_tex_array,
                ctx->queries->d_addrs_tex_array,
                ctx->queries->d_lengths_array,
                numQueries,
        ...
        hipLaunchKernelGGL(mummergpuKernel, dimGrid, dimBlock, 0 , 0, ctx->results.d_match_coords,
#if COALESCED_QUERIES
                ctx->results.d_coord_tex_array,
#endif
