...
float *GICOV_CUDA(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	...
	hipLaunchKernelGGL(GICOV_kernel, num_blocks, threads_per_block , 0, 0, grad_m, device_gicov);
...
float *dilate_CUDA(int max_gicov_m, int max_gicov_n, int strel_m, int strel_n) {
	...
	hipLaunchKernelGGL(dilate_kernel, num_blocks, threads_per_block , 0, 0, max_gicov_m, max_gicov_n, strel_m, strel_n, device_img_dilated);
